...
#VOC画像の読み込み
#引数:path(画像ファイルへのパス)
#返り値:resizeされた画像(img),w,h,c(画像のshape)
...
def main():
    ...
    #======================================================================
    # VOC2007 のデータセットへのパス
    #//:絶対パスでも相対パスでも行けるように保証
    dataset_path = "C:\Data\MachineLearning_DataSet\VOC2007\\"
    #dataset_path='C:\Users\tnktn\\'　　　　　　　　　　　　　　　　#自分用パス
    ...
    #pickle:pythonのオブジェクト(リスト等)をバイト列で保存したり復元したりする。
    #.keys():dictionaryの関数,辞書のkeyを返す
    #------------------------------------------------------------------------------------------------------
    ...
    for key in train_keys[0:n_trains]:
        ...
        image, _, _, _ = load_image_voc2007(  imagepath )
    ...
    ssd = SingleShotMultiBoxDetector(
              ...
              n_boxes = [ 4, 6, 6, 6, 6, 6 ]    # 各特徴マップにおけるデフォルトボックス数
          )
